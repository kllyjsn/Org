import { sumbleAllPeople, sumbleRelatedPeople, sumbleResolveOrg } from './sumble.js';
...
export interface RosterProvider {
  ...
  label: string;
  available(): boolean;
  fetch(domain: string, opts: { max: number; deadlineMs: number; onProgress: (n: number) => void }): Promise<RosterCandidate[]>;
}

function attr(row: Record<string, unknown>): Record<string, unknown> {
  return (row.attributes && typeof row.attributes === 'object' ? row.attributes : row) as Record<string, unknown>;
}
...
const sumbleProvider: RosterProvider = {
  id: 'sumble', label: 'Sumble', available: () => !!process.env.SUMBLE_API_KEY,
  async fetch(domain, opts) {
    ...
    return rows.flatMap((row) => {
      ...
      return [{ name, title: text(a.job_title ?? a.title), location: text(a.location), linkedin,
        email: text(a.email), managerName, managerLinkedin: null, jobLevel: text(a.job_level),
        functionHint: text(a.job_function), sourceUrl: text(a.sumble_url), raw: row }];
    });
  },
};
...
const crustdataProvider: RosterProvider = {
  id: 'crustdata', label: 'Crustdata', available: () => !!process.env.CRUSTDATA_API_KEY,
  async fetch(domain, opts) {
    ...
    while (out.length < opts.max && Date.now() < opts.deadlineMs - 1000) {
      ...
      for (const row of profiles) {
        ...
        const current = (crustNested(row, ['experience', 'employment_details', 'current']) ?? {}) as Record<string, unknown>;
        const name = crustText(basic.name ?? [basic.first_name, basic.last_name].filter(Boolean).join(' '));
        if (!name) continue;
        const linkedin = crustText(row.linkedin_url ?? crustNested(row, ['professional_network', 'linkedin_url']) ?? crustNested(row, ['professional_network', 'profile_url']));
        out.push({ name, title: crustText(current.title ?? basic.headline), location: crustText(basic.location),
          linkedin, email: crustText(row.email), managerName: null, managerLinkedin: null,
          jobLevel: crustText(current.seniority_level ?? basic.seniority_level),
          functionHint: crustText(basic.normalized_title && typeof basic.normalized_title === 'object' ? (basic.normalized_title as Record<string, unknown>).department : null),
          sourceUrl: linkedin, raw: row });
      }
...
export function rosterProviders(): RosterProvider[] { return [sumbleProvider, crustdataProvider]; }
export function configuredRosterProviders(): string[] { return rosterProviders().filter((provider) => provider.available()).map((provider) => provider.id); }
