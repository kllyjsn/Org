...
app.get('/api/workspaces/:id/members', requireAuth, async (c) => {
  ...
  const invites = await query(
    `SELECT id, email, created_at, expires_at FROM workspace_invites
     WHERE workspace_id = $1 AND accepted_at IS NULL AND expires_at > $2
     ORDER BY created_at`,
    [wsId, now()]
  );
  return c.json({ members: rows, invites });
});
...
app.post('/api/workspaces/:id/members', requireAuth, async (c) => {
  ...
  await query(
    `INSERT INTO workspace_invites
     (id, workspace_id, email, token_hash, role, invited_by, created_at, expires_at)
     VALUES ($1,$2,$3,$4,'member',$5,$6,$7)`,
    [randomUUID(), wsId, email, hashInviteToken(secret), user.id, now(), expiresAt]
  );
  ...
  const emailSent = await sendEmail({
    ...
    text: `${user.name} invited you to join ${workspaceName} on TopDown.\n\nAccept the invite: ${inviteUrl}\n\nThis link expires in 7 days.`,
    html: `<p>${escapeHtml(user.name)} invited you to join <strong>${escapeHtml(
      workspaceName
    )}</strong> on TopDown.</p><p><a href="${inviteUrl}">Accept the invite</a></p><p>This link expires in 7 days.</p>`,
  });
...
app.post('/api/invites/:token/accept', async (c) => {
  ...
  await query(
    ...
    [invite.workspace_id, user.id, invite.role, now()]
  );
  await query(
