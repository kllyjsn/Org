...
export async function createRosterSyncJob(input: {
  workspaceId: string;
  mapId: string;
  userId: string;
  domain: string;
...
export async function getRosterSyncJob(id: string) {
  return (
    (await query<RosterSyncJobRow>(
      'SELECT * FROM roster_sync_jobs WHERE id = $1',
      [id]
    ))[0] ?? null
  );
}
...
async function save(
  ...
  await query(
    ...
    [
      id,
      fields.status ?? null,
      fields.events ? JSON.stringify(fields.events) : null,
      fields.summary === undefined ? null : JSON.stringify(fields.summary),
      fields.error ?? null,
      now(),
    ]
  );
...
export async function runRosterSync(jobId: string) {
  ...
  try {
      ...
      try {
        const candidates = await provider.fetch(job.domain, {
          ...
          onProgress: (n) => {
            ...
            if (Date.now() - lastPersistAt < 1_500) return;
            lastPersistAt = Date.now();
            const snapshot = events.map((event) => ({ ...event }));
            progressSave = progressSave
              .then(() => save(jobId, { events: snapshot }))
              .catch(() => undefined);
          },
        });
        await progressSave;
        ...
        const result = await upsertRosterPeople(job.workspace_id, job.domain, candidates.map((candidate) => ({
          ...
          managerKey: candidate.managerLinkedin
            ? linkedinSlug(candidate.managerLinkedin)
          ...
          functionHint: candidate.functionHint,
        })));
    ...
    const summary =
      completed.length === 0 &&
      events.every((event) => event.status === 'skipped')
        ? { message: 'no providers configured' }
        : {
            providers: completed,
            total: await rosterCounts(job.workspace_id, job.domain),
          };
