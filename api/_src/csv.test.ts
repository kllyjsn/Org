...
test('imports Sales Navigator CSV', () => {
  ...
  assert.deepEqual(rows[0], { name: 'Jane Doe', title: 'VP Sales', linkedin: 'https://www.linkedin.com/in/jane-doe', email: null, location: 'New York', company: 'Acme', manager: null });
});
test('imports HubSpot CSV', () => {
  ...
  assert.deepEqual(rows[0], { name: 'Ada Lovelace', title: 'CTO', linkedin: null, email: 'ada@example.com', location: 'London', company: 'Example', manager: null });
});
...
test('imports manager columns', () => {
  const rows = rowsFromCsv('Name,Title,Manager\nJane Doe,VP Sales,Ada Lovelace\nJohn Smith,Account Executive,Ada Lovelace');
  assert.equal(rows[0].manager, 'Ada Lovelace');
  assert.equal(rows[1].manager, 'Ada Lovelace');
});
test('normalizes and deduplicates LinkedIn URLs', () => {
  assert.deepEqual(rowsFromLinkedinUrls(['https://www.linkedin.com/in/jane-doe-123ab', 'linkedin.com/in/jane-doe-123ab', 'https://www.linkedin.com/in/john-smith']), [
    { name: 'Jane Doe', title: null, linkedin: 'https://www.linkedin.com/in/jane-doe-123ab', email: null, location: null, company: null, manager: null },
    { name: 'John Smith', title: null, linkedin: 'https://www.linkedin.com/in/john-smith', email: null, location: null, company: null, manager: null },
  ]);
});
